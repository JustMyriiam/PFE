...
export class QuoteComponent implements OnInit {
  ...
  trackId = (item: IQuote): string => this.quoteService.getQuoteIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && QuoteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && QuoteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.quoteService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
