...
export class AgencyComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['id', 'name', 'address', 'region', 'phoneNumber', 'managerName'];
  ...
  trackId = (item: IAgency): string => this.agencyService.getAgencyIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && AgencyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && AgencyComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.agencies.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.agencyService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
