...
export class VehicleComponent implements OnInit {
  ...
  trackId = (item: IVehicle): string => this.vehicleService.getVehicleIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && VehicleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && VehicleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.vehicleService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
