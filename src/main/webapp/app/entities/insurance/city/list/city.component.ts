...
import { CityService, EntityArrayResponseType } from '../service/city.service';
...
export class CityComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['id', 'name', 'postalCode'];
  ...
  trackId = (item: ICity): string => this.cityService.getCityIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && CityComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && CityComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
